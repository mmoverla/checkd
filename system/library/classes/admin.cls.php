...
function getApplication(){
    ...
    $getApplicationSql = "SELECT id FROM application" ;
    $getApplicationResult = $this->dbclass->select($getApplicationSql);
    
    if(count($getApplicationResult) > 0) {
        $appType = array();
        foreach($getApplicationResult as $getApplicationRes){
            $appType[] = $getApplicationRes[id] ;
        }
        return $appType ;
    } else {
        return false;
	...
	function registrationNew(){
		...
		if(count($sqlRes) > 0){
			...
			do {
				...
				$checkEquipment = 'SELECT id FROM chkd_projects WHERE qr_code = "'.$equipCode.'"' ;
			...
			if($res){
				...
				$applicationType = $this->getApplication() ;
echo '<pre>' ; print_r($applicationType); exit;
                                //insert into user_app table
                                if(!empty($applicationType)){
                                    foreach($applicationType as $appType){
                                       $insertAppType = 'INSERT INTO user_app (`user_id`, `user_type`, `app_id`, `is_login`, `created`, `modified`) VALUES ("'.$user_id.'", "'.$user_type.'", "'.$appType.'", "0", "'.date('Y-m-d H:i:s').'", "'.date('Y-m-d H:i:s').'")';
                                       $insertAppTypeRes = $this->dbclass->insert($insertAppType); 
                                    }
                                }
				
                                //create Equipment type
				$equipTypeSql = 'INSERT INTO equipment_type (`company_id`, `name`, `created`, `modified` ) VALUES ("'.$user_id.'", "DEMO", "'.date('Y-m-d H:i:s').'", "'.date('Y-m-d H:i:s').'")' ; 
                                $equipTypeRes = $this->dbclass->insert($equipTypeSql);
                                
				//create work type
				$workTypeSql = 'INSERT INTO tr_work_type (`company_id`, `name`, `status`, `created`, `modified` ) VALUES ("'.$user_id.'", "DEMO", "active", "'.date('Y-m-d H:i:s').'", "'.date('Y-m-d H:i:s').'")' ; 
                                $workTypeRes = $this->dbclass->insert($workTypeSql);

				//create project for comapny admin
				$projectName = explode('@',$email) ;
				$projectName = $projectName[0] ;
				$createProject = 'INSERT INTO chkd_projects (`company_id`, `name`, `location`, `latitude`, `longitude`, `qr_code`, `meter`, `status`, `created`, `modified`) VALUES ("'.$user_id.'", "'.$projectName.'", "'.DEFAULT_LOCATION.'", "'.DEFAULT_LATITUDE.'", "'.DEFAULT_LONGITUDE.'", "'.$equipCode.'", "50000", "active", "'.date('Y-m-d H:i:s').'", "'.date('Y-m-d H:i:s').'")';
